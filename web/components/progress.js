...
export function progressCard(p) {
  ...
  const nextLine = lvl.next
    ? el("div.subtle", { style: "margin-top:6px" }, `${lvl.points} XP · ${lvl.next - lvl.points} XP to ${lvl.nextName}`)
    : el("div.subtle", { style: "margin-top:6px" }, `${lvl.points} XP · top rank reached! 👑`);

  return el("div", {},
    el("div.spread", {},
      el("div", { style: "display:flex; align-items:center; gap:10px" },
        el("span.rank", {}, `Lv ${lvl.num}`),
        el("strong", { style: "font-size:19px" }, lvl.name)),
      el("span.tag.good", {}, `${p.solved} solved`)),
...
export function badgeTile(b) {
  return el("div", { class: `badge${b.earned ? " earned" : " locked"}`, title: b.desc },
    ...
    el("div.badge-prog", {}, b.earned ? "✓ earned" : `${b.cur}/${b.target}`));
}
